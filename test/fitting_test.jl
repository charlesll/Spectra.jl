...
@testset "Optim" begin
    ...
    peaks_info = [
        ...
        (:gaussian, [10.5, 30.0, 3.0], [100.0, 100.0, 100.0], [0., 0., 0.], [Inf, Inf, Inf]), 
    ]

    result = fit_peaks(x_fit, peak + noise, peaks_info, sigma=noise, backend=:Optim)
    ...
    peaks_info = [
        # (type, initial_params, uncertainties, lower_bounds, upper_bounds)
        (:lorentzian, [10.5, 30.0, 3.0], [100.0, 100.0, 100.0], [0., 0., 0.], [Inf, Inf, Inf]),
    ...
    peaks_info = [
        # (type, initial_params, uncertainties, lower_bounds, upper_bounds)
        (:pseudovoigt, [10.5, 30.0, 3.0, 0.5], [100.0, 100.0, 100.0, 1.0], [0., 0., 0., 0.], [Inf, Inf, Inf, 1.0]),
...
@testset "Optim, multiple peaks" begin
    ...
    (:lorentzian, [10.5, 55.0, 3.0], [5.0, 10., 5.0], [0., 0., 0.], [100., 100., 50.]),
    (:pseudovoigt, [10.5, 44.0, 3.0, 0.4], [5.0, 10., 5.0, 0.05], [0., 0., 0., 0.], [100., 100., 50., 1.])
]
...
@testset "quasi Gauss Newton, multiple peaks" begin
    ...
    result = fit_peaks(x_fit, y_fit, peaks_info, sigma=noise, backend=:qGN, relax=6, maxiter=100)
