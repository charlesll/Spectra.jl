...
@testset "Correction Methods" begin
    ...
    x_long, y_long, ese_long = tlcorrection(x, y, temperature_C, laser_wavelength; correction="long")
    @test all(.!isnan.(y_long))

    # Test "galeener" correction method
    x_galeener, y_galeener, ese_galeener = tlcorrection(x, y, temperature_C, laser_wavelength; correction="galeener")
    @test all(.!isnan.(y_galeener))

    # Test "hehlen" correction method with density input
    x_hehlen, y_hehlen, ese_hehlen = tlcorrection(x, y, temperature_C, laser_wavelength; correction="hehlen", density=2200.0)
...
@testset "Normalisation Options" begin
    ...
    _, y_intensity, _ = tlcorrection(x, y, temperature_C, laser_wavelength; normalisation="intensity")
    @test isapprox(maximum(y_intensity), 1.0; atol=1e-6)

    # Test minmax normalization
    _, y_minmax, _ = tlcorrection(x, y, temperature_C, laser_wavelength; normalisation="minmax")
    @test isapprox(minimum(y_minmax), 0.0; atol=1e-6) && isapprox(maximum(y_minmax), 1.0; atol=1e-6)

    # Test no normalization (output should match corrected values without scaling)
    _, y_no_norm, _ = tlcorrection(x, y, temperature_C, laser_wavelength; normalisation="no")
end
