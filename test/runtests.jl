...
x_long,long_res,eselong = long([x_for_long y_for_long],23.0,514.532) # using the function
t0 = nu0.^3.*nu./((nu0-nu).^4)
t1= 1 - exp(-h_plank.*c_light.*nu./(k_bolt.*T)) # c in m/s  : t1 dimensionless
long_calc= y_for_long.*t0.*t1 # pour les y
long_calc = long_calc./trapz(x,long_calc) # area normalisation
