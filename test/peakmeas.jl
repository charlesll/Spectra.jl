...
@testset "area_peaks Tests" begin
    x = collect(-10000:0.01:20000) # we need a broad and high resolution X axis to check with trapz...
    peak_infos = [
        Dict(:type => :gaussian, :amplitude => 1.0, :center => 1.0, :hwhm => 1.0),
        Dict(:type => :lorentzian, :amplitude => 1.0, :center => 1.0, :hwhm => 1.0),
        Dict(
            :type => :pseudovoigt,
            :amplitude => 1.0,
            :center => 1.0,
    ...
    @test isapprox(a_pv, areas[3], atol=1e-3)
    @test isapprox(a_pearson7, areas[4], atol=1e-3)
end

@testset "centroid Tests" begin
    #### centroid test
    x = collect(0.0:1.0:100.0)
    y_peak = gaussian(x, 1.0, 50.0, 10.0)
...
@testset "find_peaks Tests" begin
    ...
    @test_throws ErrorException find_peaks(
        x_invalid, y_invalid; smoothing=false, window_size=10
    )
